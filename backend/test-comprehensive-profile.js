...
async function comprehensiveProfileTest() {
  try {
    ...
    console.log(
      "Updated name:",
      `${updateResponse.data.data.user.firstName} ${updateResponse.data.data.user.lastName}`
    );
    ...
    const leaderResponse = await axios.put(
      "http://localhost:5001/api/v1/users/profile",
      leaderUpdate,
      {
        headers: {
    ...
    console.log(
      "Leader status:",
      leaderResponse.data.data.user.isAtCloudLeader
    );
    ...
    for (const testCase of edgeCases) {
      try {
        ...
        if (!testCase.shouldPass) {
          ...
          console.log(
            `❌ ${testCase.name}: Unexpectedly failed:`,
