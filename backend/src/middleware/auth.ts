...
export class TokenService {
  ...
  private static readonly ACCESS_TOKEN_EXPIRE = process.env.JWT_ACCESS_EXPIRE || '15m';
  ...
  static generateTokenPair(user: IUser) {
    ...
    const refreshToken = this.generateRefreshToken({ userId: (user._id as any).toString() });
...
export const verifyEmailToken = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
export const verifyPasswordResetToken = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
