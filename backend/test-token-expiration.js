...
async function testTokenExpiration() {
  try {
    ...
    if (loginResponse.data.success) {
      ...
      const timeDifference = (expiresAt.getTime() - now.getTime()) / (1000 * 60 * 60);
      ...
      if (timeDifference >= 1.9 && timeDifference <= 2.1) {
        ...
        console.log('❌ Token expiration is not 2 hours:', timeDifference.toFixed(2), 'hours');
      }
      
      // Test the token by making an authenticated request
      console.log('\n🔐 Testing token authentication...');
      const profileResponse = await axios.get('http://localhost:5001/api/v1/auth/profile', {
        headers: {
