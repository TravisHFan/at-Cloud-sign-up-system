...
describe("LockService", () => {
  ...
  describe("Lock Contention and Serialization", () => {
    it("should serialize operations with the same lock key", async () => {
      ...
      const operation1 = vi.fn().mockImplementation(async () => {
        ...
        // Remove timing delay - test should be deterministic
    ...
    it("should handle multiple operations queued on same lock", async () => {
      ...
      const op1 = createOperation("A");
      const op2 = createOperation("B");
      const op3 = createOperation("C");
  ...
  describe("Timeout Handling", () => {
    it("should timeout when lock is held too long", async () => {
      // Create a promise we can control - never resolves unless we tell it to
      let resolveSlowOperation: () => void;
      ...
      const slowOperation = vi.fn().mockImplementation(
        () =>
          new Promise<string>((resolve) => {
            lockAcquired = true;
            resolveSlowOperation = () => resolve("slow");
          })
      );

      // Start the slow operation (this will hold the lock indefinitely)
      const slowPromise = testLockService.withLock(
        "timeout-key",
        slowOperation
      );

      // Wait until we're certain the first lock is acquired
      await new Promise((resolve) => {
        const checkLock = () => {
          if (lockAcquired) {
            resolve(void 0);
          } else {
            process.nextTick(checkLock);
          }
        };
        checkLock();
      });

      // Try to acquire same lock with reasonable timeout that's reliable across environments
      const timeoutPromise = testLockService.withLock(
        "timeout-key",
        () => Promise.resolve("fast"),
        100 // Longer timeout for reliability
      );

      await expect(timeoutPromise).rejects.toThrow(
        "Lock timeout after 100ms for key: timeout-key"
      );

      // Clean up: resolve the slow operation
      resolveSlowOperation!();
      await slowPromise;
    });

    it("should use default timeout when not specified", async () => {
      // Create a controllable promise that won't resolve unless we tell it to
      ...
      const operation = vi.fn().mockImplementation(
        () =>
          new Promise<string>((resolve) => {
            lockAcquired = true;
            resolveOperation = () => resolve("op");
          })
      );

      // Start the first operation
      const promise = testLockService.withLock("default-key", operation);

      // Wait until we're certain the lock is acquired
      await new Promise((resolve) => {
        const checkLock = () => {
          if (lockAcquired) {
            resolve(void 0);
      ...
      const timeoutPromise = testLockService.withLock(
        ...
        150 // Reasonable timeout for test reliability
      );

      await expect(timeoutPromise).rejects.toThrow("Lock timeout after 150ms");
    ...
    it("should handle custom timeout values", async () => {
      ...
      // Start the first operation
      const promise = testLockService.withLock("custom-key", operation);
      ...
      // Test custom timeout - should fail
      const timeoutPromise = testLockService.withLock(
        "custom-key",
        () => Promise.resolve("quick"),
        120 // Custom timeout that's reliable
      );

      await expect(timeoutPromise).rejects.toThrow("Lock timeout after 120ms");

      // Clean up
      resolveOperation!();
      await promise;

      // Test successful operation with different key (no contention)
      const result = await testLockService.withLock(
        "custom-key-2",
