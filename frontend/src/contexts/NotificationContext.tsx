...
export function NotificationProvider({ children }: { children: ReactNode }) {
  ...
  const addSystemMessage = async (
    ...
    try {
      const result = await systemMessageService.createSystemMessage(message);
      if (result) {
        // Since the service returns boolean, create a mock message for local state
  ...
  const addAutoSystemMessage = async (
    ...
    try {
      const result = await systemMessageService.createSystemMessage({
        ...message,
        // Remove isAutoGenerated as it's not part of the SystemMessage type
      });
      if (result) {
        // Since the service returns boolean, create a mock message for local state
        const newMessage: SystemMessage = {
